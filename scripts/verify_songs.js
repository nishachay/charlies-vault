...
(async () => {
  ...
  for (const s of catalog.songs) {
    if (!seen.has(s.youtubeId)) { seen.add(s.youtubeId); unique.push(s.youtubeId); }
  }
  console.log(`[verify] probing ${unique.length} unique videos (catalog has ${catalog.songs.length} rows)...`);
  ...
  const enrichedSongs = catalog.songs.map(s => {
    ...
    return { ...s, videoStatus: v.status, realTitle: v.title || null, realAuthor: v.author || null };
  });
