...
// Refresh cycle: probe stale songs, persist status, return summary.
async function runRefresh(db, {
  apiKey, maxAgeMs, force = false, concurrency = 5, fetchImpl = fetch,
} = {}) {
  const { setSongStatus, touchChecked, staleSongs } = require('../src/models');
  ...
  const summary = { checked: results.length, active: 0, dead: 0, private: 0, errors: 0 };
